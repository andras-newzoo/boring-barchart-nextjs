...
var BarChart = function BarChart(_ref) {
  ...
  updateDims = function updateDims() {
    ...
    chartArea.selectAll('rect').attr('width', function (d) {
      return xScale(d.share);
    }).attr('y', function (d) {
      return yScale(d.group);
    }).attr('height', yScale.bandwidth());
    chartArea.selectAll('.value').attr('x', function (d) {
  ...
  var createUpdateValueText = function createUpdateValueText() {
    var _storedValues$current4 = storedValues.current,
        ...
        xScale = _storedValues$current4.xScale;
    ...
    removeElement(texts);
    texts.enter().append('text').attr('class', 'value').attr('x', 0).attr('y', centerText).attr('opacity', 0).text(0).merge(texts).transition().duration(updateDuration).ease(d3_ease__WEBPACK_IMPORTED_MODULE_11__["easeCubicInOut"]).attr('x', function (d) {
      return xScale(d.share) - 3;
    }).tween('text', function (d, i, n) {
      return Object(_utils__WEBPACK_IMPORTED_MODULE_9__["numberTween"])(d, i, n, 'share', '.1%');
    }).attr('opacity', 1);
    storedValues.current = Object(_babel_runtime_corejs2_helpers_esm_objectSpread__WEBPACK_IMPORTED_MODULE_0__["default"])({}, storedValues.current, {
  ...
  var createUpdateVariableText = function createUpdateVariableText() {
    var _storedValues$current5 = storedValues.current,
        chartArea = _storedValues$current5.chartArea,
        yScale = _storedValues$current5.yScale;
    ...
    texts.enter().append('text').attr('class', 'variable').attr('x', 1).attr('y', yPos).attr('opacity', 0).text(function (d) {
      return d.group;
    }).merge(texts).transition().duration(updateDuration).ease(d3_ease__WEBPACK_IMPORTED_MODULE_11__["easeCubicInOut"]).attr('y', yPos).attr('opacity', 1);
    storedValues.current = Object(_babel_runtime_corejs2_helpers_esm_objectSpread__WEBPACK_IMPORTED_MODULE_0__["default"])({}, storedValues.current, {
      yPos: yPos
    });
  };
  ...
  var removeElement = function removeElement(el) {
    return el.exit().transition().duration(updateDuration).ease(d3_ease__WEBPACK_IMPORTED_MODULE_11__["easeCubicInOut"]).attr('y', dims.height * 2).remove();
  };
...
//# sourceMappingURL=furniture-bank-2019.js.e846c64394ec20f3d9b7.hot-update.js.map