...
var PercentageChart = function PercentageChart(_ref) {
  ...
  Object(react__WEBPACK_IMPORTED_MODULE_1__["useEffect"])(function () {
    if (init && prevData !== data) {
      if (prevCount !== fullCount) {
        resetChart(dims.width); // updateRect(resetDelay * 2);
      } else {
        updateRect();
      }
    }
  ...
  resetChart = function resetChart(resetPoint) {
    ...
    chartArea.select("rect").transition().duration(updateDuration / 2).ease(d3_ease__WEBPACK_IMPORTED_MODULE_6__["easeCubicInOut"]).attr("width", toEnd ? dims.width : 0).transition().delay(resetDelay).duration(0).attr("width", toEnd ? 0 : 1).transition().delay(updateDuration / 2).duration(updateDuration / 2).ease(d3_ease__WEBPACK_IMPORTED_MODULE_6__["easeCubicInOut"]).attr("width", dims.width * (data - fullCount));
  };

  updateRect = Object(react__WEBPACK_IMPORTED_MODULE_1__["useCallback"])(function () {
    var delay = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
    var chartArea = storedValues.current.chartArea;
    chartArea.select("rect").transition("update").delay(delay).duration(updateDuration).ease(d3_ease__WEBPACK_IMPORTED_MODULE_6__["easeCubicInOut"]).attr("width", dims.width * (data - fullCount));
...
//# sourceMappingURL=furniture-bank-2019.js.03cafa0d1abe82f92b17.hot-update.js.map